...
###############################################################################
#
# done. READ these instructions, ASKING QUESTIONS as needed.
#
...
# -----------------------------------------------------------------------------
# done. Follow the INSTRUCTIONS AT THE TOP OF THIS MODULE
...
def broken_3(n, point, length, distance_between_lines, window):
    ...
    for k in range(n):
        ...
        window.render(0.5)
        a = rg.Point(point.x+(k+1)*distance_between_lines, point.y)
        b=rg.Point(point.x+(k+1)*distance_between_lines,point.y+length)
...
def broken_7(n, point, length, distance_between_lines, window):
    ...
    for k in range(n):
        ...
        left = rg.Point(point.x, point.y + (k+1)*distance_between_lines)
        right = rg.Point(point.x+length, point.y + (k+1)*distance_between_lines)
        window.render(0.5)
    window.close_on_mouse_click()
